...
interface LendsqrLogoProps extends React.SVGProps<SVGSVGElement> {
  width?: number
  height?: number
}

/**
 * Renders a customizable Lendsqr logo (svg)
 *
 * @param {number} width -  The width of the logo (optional)
 * @param {number} height - The height of the logo (optional)
 *
 * @returns {JSX.Element} - The Lendsqr logo
 *
 * @example
 * // Renders the logo with the default size (174 x 36).
 * <LendsqrLogo/>
 *
 * @example
 * // Renders the logo with a custom size (200 x 50).
 * <LendsqrLogo width={200} height={50}/>
 *
 * @example
 * // Renders the logo with a custom size and style.
 * <LendsqrLogo width={200} height={50} className="test-class"/>
 *
 */

export default function LendsqrLogo(
  props: Readonly<LendsqrLogoProps>
): JSX.Element {
  /*
  I decided to make this a component rather than pasting the code(svg) wherever 
  it was needed or using it as an image so it could be more flexible across
  multiple pages.

  I also consired a case where the logo's color changes with the theme.
  I believe this would be easier to implement if this is a component.
  */

  // Destructure props
  const { width = 174, height = 36, ...rest } = props

  // Ensure width and height are valid numbers
  const validWidth = typeof width === 'number' && !isNaN(width) ? width : 174
  const validHeight = typeof height === 'number' && !isNaN(height) ? height : 36
  ...
  return (
    <svg
      ...
      height={`${validHeight}`}
      role="img"
      aria-label="Lendsqr Logo"
      viewBox={`0 0 174 36`} // Keep constant so the logo's ratio doesn't change
      fill="none"
